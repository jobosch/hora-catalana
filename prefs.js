...
export default class HoraCatalanaPrefs extends ExtensionPreferences {
  fillPreferencesWindow(window) {
    ...
    const showDateRow = new Adw.SwitchRow({
      ...
      subtitle:
        "Activa o desactiva la visualització de la data abans de l’hora.",
    ...
    colorButton.set_rgba(
      new Gdk.RGBA({
        red:
          parseInt(settings.get_string("font-color").substring(1, 3), 16) / 255,
        green:
          parseInt(settings.get_string("font-color").substring(3, 5), 16) / 255,
        blue:
          parseInt(settings.get_string("font-color").substring(5, 7), 16) / 255,
        alpha: 1,
      }),
    );
    ...
    colorButton.connect("notify::rgba", (widget) => {
      ...
      const hex = `#${Math.round(rgba.red * 255)
        .toString(16)
        .padStart(2, "0")}${Math.round(rgba.green * 255)
        .toString(16)
        .padStart(2, "0")}${Math.round(rgba.blue * 255)
        .toString(16)
        .padStart(2, "0")}`;
      settings.set_string("font-color", hex);
    });

    // Entrada pel separador
    const separatorRow = new Adw.EntryRow({
      title: _("Separador (màx. 3)"),
      text: settings.get_string("separator"),
      max_width_chars: 3,
    });
    separatorRow.connect("notify::text", (widget) => {
      const text = widget.text.slice(0, 3);
      if (text !== widget.text) widget.set_text(text);
      settings.set_string("separator", text);
    });
